...
Scanner::Scanner(Buffer& buffer, Rules& rules) 
	: m_buffer(buffer), m_rules(rules), lexeme(""), state(0) 
{
	ruleSet = m_rules.getRules();
}
...
void Scanner::scan()
{
	...
	processNextChar();	// Process also the last character
...
void Scanner::processNextChar()
{
	int input = m_rules.characterCategory(c);
	// State action with the current input is MOVE and the state where to move from this state with the given input is not -1.
